...
public class ServicioFleteControllerTest {
	...
	@MockBean
	private IServicioFleteService fleteService;
	...
	void indexTest() throws Exception {
		...
		fletes.add(flete1);
		fletes.add(flete2);
		when(fleteService.findAll()).thenReturn(fletes);
		
		// when
		mvc.perform(get("/apiEstacionamiento/servicioFlete").contentType(MediaType.APPLICATION_JSON))
		//then
		.andExpect(status().isOk())
		.andExpect(content().contentType(MediaType.APPLICATION_JSON))
		.andExpect(jsonPath("$.ok").value(true))
		.andExpect(jsonPath("$.servicioFlete").isNotEmpty());
		
		verify(fleteService).findAll();
	}
    ...
    void indexTestNoValido() throws Exception {
        ...
        when(fleteService.findAll()).thenThrow(new DataAccessException("...") {});
